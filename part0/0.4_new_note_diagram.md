...
sequenceDiagram 
  ...
  note over browser: The browser starts execution the javaScript code that fetches the JSON from the server <br> sends GET request for notes data

  browser->>server: GET /exampleapp/data.json
  server-->>browser: Return notes data as JSON [{content: ..., date: ...}, ...]

  note over browser: Browser executes event handler callback function <br> that rerenders webpage with notes data
